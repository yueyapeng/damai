...
public class HttpUtil {
	...
	// connectionTimeout TODO 配置文件统一管理
	private static int MAX_TIMEOUT = 5000;
	// socketTimeout TODO 配置文件统一管理
	 ...
	 * @return
	 */
	public static RequestResult doPost(String url, JSONObject json) {
	 ...
	 * @return
	 */
	public static RequestResult doPost(String traceId, String url, Map<String, Object> params,
		...
		try {
			...
			log.error("{}请求{}地址发生异常:", traceId, url, e);
		} finally {
			if (response != null) {
				try {
	...
	// 二进制专用HTTPS（单独key）
	public static RequestResult doBytePost(String traceId, String url, Map<String, Object> params, String key,
			String fileName) {
			...
			for (Map.Entry<String, Object> entry : params.entrySet()) {
				if (entry.getValue() != null) {
					if (StringUtils.isNotBlank(key) && key.equals(entry.getKey())) {
						InputStream input = new ByteArrayInputStream((byte[]) entry.getValue());
						builder.addBinaryBody(entry.getKey(), input, ContentType.create("multipart/form-data"),
								fileName + ".jpg");
					} else {
						// 添加参数和参数值
	...
	// 二进制专用HTTPS（多key）
	public static RequestResult doBytePostListKey(String traceId, String url, Map<String, Object> params, List<String> listKey,
										   String fileName) {
		CloseableHttpClient httpClient = HttpClients.custom().setSSLSocketFactory(createSSLConnSocketFactory())
				.setConnectionManager(connMgr).setDefaultRequestConfig(requestConfig).build();
		RequestResult requestResult = new RequestResult();
	 ...
	 * @return
	 */
	public static RequestResult doPost(String traceId, String url, JSONObject json, Map<String, String> headers) {
		CloseableHttpClient httpClient = HttpClients.custom().setDefaultRequestConfig(requestConfig).build();
		RequestResult requestResult = new RequestResult();
		String httpStr = null;
		HttpPost httpPost = new HttpPost(url);
